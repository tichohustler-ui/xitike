...
class _XitikePageState extends State<XitikePage> {
  ...
  final mensagem = 'Venha participar do grupo $grupo no XITIKE! 🔗 https://xitike.com/convite/$grupo/$fundadorRecrutadorId';
  
  // URL corrigido para WhatsApp
  final url = 'https://api.whatsapp.com/send?text=${Uri.encodeComponent(mensagem)}';
  ...
  try {
    if (await canLaunchUrl(Uri.parse(url))) {
      ...
      // Fallback: tentar com o link antigo
      final fallbackUrl = 'whatsapp://send?text=${Uri.encodeComponent(mensagem)}';
      if (await canLaunchUrl(Uri.parse(fallbackUrl))) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Mensagem copiada! Cole no WhatsApp manualmente.'),
            backgroundColor: Colors.green,
          ),
        );
      }
...
void _compartilharFacebook(String grupo, String fundadorRecrutadorId) async {
  ...
  final shareUrl = 'https://www.facebook.com/sharer/sharer.php?u=${Uri.encodeComponent(link)}';
...
void _compartilharSMS(String grupo, String fundadorRecrutadorId) async {
  ...
  try {
    if (await canLaunchUrl(Uri.parse(url))) {
      ...
      final fallbackUrl = 'sms:/?body=${Uri.encodeComponent(mensagem)}';
      if (await canLaunchUrl(Uri.parse(fallbackUrl))) {
        await launchUrl(Uri.parse(fallbackUrl));
      } else {
        await Clipboard.setData(ClipboardData(text: mensagem));
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Mensagem copiada! Cole no SMS manualmente.'),
            backgroundColor: Colors.green,
          ),
        );
      }
